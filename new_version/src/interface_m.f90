module interface_m

    use constant_m
    use utility_m
    implicit none

    interface

        module pure character(len=20) function euler0001()
        end function euler0001

        module pure character(len=20) function euler0002()
        end function euler0002

        module pure character(len=20) function euler0003()
        end function euler0003

        module pure character(len=20) function euler0004()
        end function euler0004

        module pure character(len=20) function euler0005()
        end function euler0005

        module pure character(len=20) function euler0006()
        end function euler0006

        module pure character(len=20) function euler0007()
        end function euler0007

        module pure character(len=20) function euler0008()
        end function euler0008

        module pure character(len=20) function euler0009()
        end function euler0009

        module pure character(len=20) function euler0010()
        end function euler0010

        module pure character(len=20) function euler0011()
        end function euler0011

        module pure character(len=20) function euler0012()
        end function euler0012

        module pure character(len=20) function euler0013()
        end function euler0013

        module pure character(len=20) function euler0014()
        end function euler0014

        module pure character(len=20) function euler0015()
        end function euler0015

        module pure character(len=20) function euler0016()
        end function euler0016

        module pure character(len=20) function euler0017()
        end function euler0017

        module pure character(len=20) function euler0018()
        end function euler0018

        module pure character(len=20) function euler0019()
        end function euler0019

        module pure character(len=20) function euler0020()
        end function euler0020

        module pure character(len=20) function euler0021()
        end function euler0021

        module pure character(len=20) function euler0022()
        end function euler0022

        module pure character(len=20) function euler0023()
        end function euler0023

        module pure character(len=20) function euler0024()
        end function euler0024

        module pure character(len=20) function euler0025()
        end function euler0025

        module pure character(len=20) function euler0026()
        end function euler0026

    end interface

end module interface_m