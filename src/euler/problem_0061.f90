submodule(interface_m) euler_problem_0061_m
    implicit none
...
contains
    ...
    pure integer(i32) function answer()
        type(variant_array_t) :: polygonals(3:8)
        type(variant_array_t), allocatable :: array(:)
        logical :: avail
        integer(i32) :: i(6), k
    ...
    pure subroutine get_cyclic(x, y)
        type(variant_array_t), allocatable, intent(inout) :: x(:)
        integer(i32), intent(in) :: y(:)
        type(variant_array_t) :: temp(99)
        integer(i32) :: i, j, k, s
    ...
    pure subroutine get_polygonals(p)
        type(variant_array_t), intent(out) :: p(3:8)
        integer(i32) :: i, j
...
end submodule euler_problem_0061_m
